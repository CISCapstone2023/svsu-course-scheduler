    ...
    .mutation(async ({ ctx, input }) => {
      const newCourse = await ctx.prisma.course.create({
        data: {
          ...
          section_id: input.course.section_id,
          revision_tuid: input.course.revision_tuid,
          term: input.course.term,
          semester_summer: input.course.semester_summer,
          semester_fall: input.course.semester_fall,
          semester_winter: input.course.semester_winter,
          semester_spring: input.course.semester_spring,
          div: input.course.div,
          department: input.course.department,
          subject: input.course.subject,
          course_number: input.course.course_number,
          section: input.course.section,
          start_date: input.course.start_date,
          end_date: input.course.end_date,
          start_time: input.course.start_time,
          end_time: input.course.end_time,
          credits: input.course.credits,
          title: input.course.title,
          status: input.course.status,
          faculty_tuid: input.course.faculty_tuid,
          instruction_method: input.course.instruction_method,
          capacity: input.course.capacity,
          original_state: input.course.original_state,
          state: input.course.state,
          location: input.course.location,
          createdAt: input.course.createdAt,
          updatedAt: input.course.updateAt,
          // faculty: {
          //   create:
          // }
          // notes: input.course.notes,
          // locations: input.course.locations,
        },
      });

      const courseConnection = await ctx.prisma.scheduleRevision.update({
        where: {
          tuid: input.tuid,
        },
        data: {
